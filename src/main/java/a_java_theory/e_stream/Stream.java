package a_java_theory.e_stream;
...
//        Собирание результатов
    ...
    @Test
    public void difBetweenColletorsToListAndToList() {

        //toList() -  Возвращает неизменяемый список (нельзя изменить элементы списка).
        //Collectors.toList() -  Возвращает изменяемый список (можно добавлять, удалять, изменять элементы)
        ...
        //Изначально здесь было так, в таком виде с toList работать не будет ImmutableCollections (UnsupportedOperationException)
//        List<String> clearedList = list.stream().filter(i -> !i.isEmpty()).toList();
        List<String> clearedList = list.stream().filter(i -> !i.isEmpty()).collect(Collectors.toList());
        clearedList.add("333");
